...
test("Single ticket booking is eligible for refund", async ({ page }) => {
  ...
  await page.getByTestId("event-card book-now-btn").first().click();
  await page.getByTestId("customerName").fill("JJ");
  await page.getByTestId("customer-email").fill("jj@jmail.com");
  ...
  const allBookingCards = page.getByTestId("booking-card").first();
  await expect(allBookingCards).toBeVisible();
  await allBookingCards.getByRole("button", { name: "View Details" }).click();
  await page.locator(".text-base").last().isVisible();
